...
describe("Given a getByIdDestination", () => {
  describe("When it's invoke and receives a id of a destination which exist", () => {
    test("Then it should return the destination", async () => {
      ...
      const resultDestination = await result.current.getByIdDestination(mockId);

      expect(resultDestination).toStrictEqual(mockDestination);
    });
  });
  ...
  describe("When it's invoke and receives a id of a destination which doesn't exist", () => {
    test("Then it should invoke dispatch with openModalActionCreator", async () => {
      ...
      // eslint-disable-next-line testing-library/no-await-sync-query
      await result.current.getByIdDestination(mockId);
